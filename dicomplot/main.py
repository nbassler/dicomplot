...
import matplotlib.ticker as ticker  # Import ticker module for setting tick locators
...
def get_spot_map(dicom_object, ion_beam_sequence_index=0, ion_control_point_sequence_index=0):
    ...
    """
    logger.info(f"Getting spot map from dicom object {ion_beam_sequence_index}, {ion_control_point_sequence_index}")
...
def plot_map(field_index, energy_layer_index, maps, ax, cbar, fig, max_weight):
    ...
    if cbar:
        cbar.remove()
    cbar = fig.colorbar(scatter, ax=ax, label='Spot Weight', orientation='vertical')
    ...
    ax.set_title(f"Field {field_index + 1}, Energy Layer {energy_layer_index + 1} - Beam Energy: {energy if energy != -1 else 'N/A'} MeV")
...
def create_interactive_plot(maps, max_weight):
    ...
    cbar = plot_map(field_index[0], energy_layer_index[0], maps, ax, cbar, fig, max_weight)

    # Define button click events, including passing max_weight
    def next_field(event):
        nonlocal cbar
        field_index[0] = (field_index[0] + 1) % len(maps)
        energy_layer_index[0] = 0  # Reset energy layer index when changing fields
        cbar = plot_map(field_index[0], energy_layer_index[0], maps, ax, cbar, fig, max_weight)
    ...
    def next_layer(event):
        nonlocal cbar
        energy_layer_index[0] = (energy_layer_index[0] + 1) % len(maps[field_index[0]])
        cbar = plot_map(field_index[0], energy_layer_index[0], maps, ax, cbar, fig, max_weight)
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description='Plot DICOM proton therapy treatment plans.')
    parser.add_argument('inputfile', help='input filename', type=str)
    parser.add_argument('-v', '--verbosity', action='count', default=0,
                        help='Increase output verbosity.')
    parsed_args = parser.parse_args(args)

    logging.basicConfig(level=logging.INFO if parsed_args.verbosity else logging.WARNING)

    dicom_object = pydicom.read_file(parsed_args.inputfile)
